$(document).ready(function() {
    ...
    $(".search-input").keyup(function() {
        ...
        $.getJSON("https://www.omdbapi.com/?", { apikey: "5de025a5", s: search }, function(data) {
            if (data.Search !== undefined) {
                $.each(data.Search, function(index, value) {
                    if (index < 2) {
                        $.getJSON("https://www.omdbapi.com/?", { apikey: "5de025a5", i: value.imdbID }, function(movieData) {
                            if (movieData) {
                                ...
                                results += '<div>Stars: '+ movieData.Actors.split(",").slice(0, 3) + ' | <a href="https://www.imdb.com/">Show All »</a></div>'
                                results += '</div>'
                                results += '<div class="my-3">'
                                results += '<div>'+ movieData.Plot.slice(0, 100) + '... <a href="https://www.imdb.com/search">Details »</a></div>'
                                results += '</div>'
                                results += '</div>'
                                results += "</div>"
    ...
    $("#show-more").click(function(e) {
        ...
        $.getJSON("https://www.omdbapi.com/?", { apikey: "5de025a5", s: search }, function(listData) {
            if (/Mobi|Android/i.test(navigator.userAgent)) {
            ...
            if (listData.Search !== undefined) {
                $.each(listData.Search, function(index, value) {
                    $.getJSON("https://www.omdbapi.com/?", { apikey: "5de025a5", i: value.imdbID }, function(listMovieData) {
                        if (listMovieData) {
